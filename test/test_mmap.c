...
int main(int argc, char **argv) {
    int      fd;
    uint8_t *full_map;
    char     path[] = "testrand.txt";
    X(  fd=open(path,O_RDWR|O_NONBLOCK|O_CREAT|O_APPEND, S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP)) X_FATAL;
    //X(  fstat(fd, &filestat)                                                             ) X_RAISE(-1);
    //curr_size = filestat.st_size;
    //mfd->map_size = MAX(curr_size, gx_pagesize);
    Xm( full_map = mmap(NULL, gx_pagesize << 1, PROT_READ|PROT_WRITE, MAP_SHARED, fd,0)) X_FATAL;

    printf("%x | %x\n", full_map[0], full_map[gx_pagesize]);
    
    //Xm( mremap(full_map+gx_pagesize, gx_pagesize, gx_pagesize << 1, 0)) X_FATAL;
    Xm( full_map = mremap(full_map, gx_pagesize << 1, gx_pagesize * 3, MREMAP_MAYMOVE)) X_FATAL;
