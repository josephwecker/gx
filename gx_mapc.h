 ...
 * same mmipc will be immediately notified about new content (via a fd that can
 ...
 *
 * If an mmipc is marked as not being persistent and it is observed that the
 * path is on a tmpfs / ram-disk based filesystem (like /dev/shm) mmipc can go
...
//------------------------------------------------------------------------------
/// Holds the state for an mmipc- including memory-mapped data pointers.
typedef gx_mmipc {
...
static GX_OPTIONAL gx_mmipc_open_w(gx_mmipc *mm, const char *path, int persistent, ) {
